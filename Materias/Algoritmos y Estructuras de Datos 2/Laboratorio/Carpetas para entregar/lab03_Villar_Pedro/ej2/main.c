...
int main(int argc, char *argv[]) {
    ...
    printf("Las temperaturas máximas registradas por año son:\n");
    for (unsigned int i = 0; i < YEARS; i++) {
        printf("Año %d: %d\n", i + FST_YEAR, max_temps[i]);
    }

    month_t max_rainfalls[YEARS];
    cant_con_mas_preci(array, max_rainfalls);
    printf("Los meses con mayor precipitación registrada por año son:\n");
    for (unsigned int i = 0; i < YEARS; i++) {
        printf("Año %d: %d\n", i + FST_YEAR, max_rainfalls[i]);
    }
    array_dump(array);
